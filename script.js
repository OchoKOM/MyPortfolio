...
const LoadText = () =>{
    setTimeout(()=>{
        Text.textContent = "CSS Student";
    },0);
    setTimeout(()=>{
        Text.textContent = "Freelancer";
    },4000);
    setTimeout(()=>{
        Text.textContent = "Web Developper";
    },8000);
}
LoadText();
...
const moonIon = document.querySelector("#them-btn");

var dark = window.matchMedia("(prefers-color-scheme: dark)").matches

if(dark){
    window.addEventListener("load", toggleThem)
}
window.matchMedia("(prefers-color-scheme: dark)").onchange = (e)=>{
    ...
    if(colorScheme === "dark"){
        if (moonIon.classList.contains('fa-moon')) {
            moonIon.classList.replace('fa-moon', 'fa-sun')
        }
        document.body.classList.add('active');
    }else{
        if (moonIon.classList.contains('fa-sun')) {
            moonIon.classList.replace('fa-sun', 'fa-moon')
        }
        document.body.classList.remove('active');
    }
}

moonIon.addEventListener('click',toggleThem)
function toggleThem(){
    if (moonIon.classList.contains('fa-sun')) {
        moonIon.classList.replace('fa-sun', 'fa-moon')
    }else{
